...
class Crypto
{
	...
	public static function encrypt($value, string $passphrase)
	{
		...
		$encrypted_data = openssl_encrypt(json_encode($value), self::CIPHER, $key, true, $iv);
	...
	public static function decrypt(string $jsonStr, string $passphrase)
	{
		...
		$data = openssl_decrypt($ct, self::CIPHER, $key, true, $iv);
	...
	public function uniqueId($length = 8) {
		...
		//return substr(base64_encode(openssl_random_pseudo_bytes('32')), 0, 8);
	}
